...
function responseHandler(callback) {
     function handler(error, data, response) {
        if (error) {
            var err = new Error('HTTP Error ' + error.statusCode + ': ' +
...
Twitter.prototype.createFriendship = function(params, callback) {
    if (!params || !(params.user_id || params.screen_name)) {
    ...
    if (typeof params.follow === 'undefined') {
        params.follow = true;
    }
...
// Alias follow to createFriendship
Twitter.prototype.follow = Twitter.prototype.createFriendship

Twitter.prototype.existsFriendship = function(userA, userB, callback) {
...
Twitter.prototype.geoSimilarPlaces = function(lat, lng, name, params, callback) {
    ...
    if (typeof lat !== 'number' || typeof lng !== 'number' || !name) {
        callback(new Error('You must specify latitude, longitude (as numbers) and name.'));
    }
...
Twitter.prototype.geoReverseGeocode = function(lat, lng, params, callback) {
    ...
    if (typeof lat !== 'number' || typeof lng !== 'number') {
        callback(new Error('You must specify latitude and longitude as numbers.'));
    }
...
Twitter.prototype._getUsingCursor = function(url, params, callback) {
    ...
    if (!key)
        callback(new Error('Results key must be provided to _getUsingCursor().'));
    delete params.key;
